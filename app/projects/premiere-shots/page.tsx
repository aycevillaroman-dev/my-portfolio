// app/projects/premiere-shots/page.tsx
"use client";
...
const project = {
  ...
  subtitle: "Web Booking Reservation System",
  publishedAt: "2024-06-01",
  summary:
    "A full-stack web booking system for a photography service — letting clients browse packages, schedule sessions, and manage reservations through a clean, form-validated interface backed by PHP and MySQL.",
  overview: `Premiere Shots is a web-based booking and reservation system built for a photography service of the same name. The core problem it solves is straightforward: clients needed a way to schedule sessions online without back-and-forth messages, and the business needed a reliable way to store and manage those bookings.

The frontend is built with HTML, CSS, and vanilla JavaScript — deliberately lightweight, since the goal was a fast, accessible interface that works across devices without heavy framework overhead. Forms are validated client-side in JavaScript before submission, catching errors early and giving users immediate feedback.

The backend is powered by PHP, handling routing, form processing, session management, and all server-side validation. Data lives in a MySQL database managed through XAMPP during development. The schema links users, available service packages, time slots, and reservation records through normalized relational tables — keeping queries efficient and the data model easy to extend.

The booking flow was designed to minimize friction: a user selects a package, picks an available date and time, fills in their details, and receives a confirmation. On the admin side, reservations are visible in a simple dashboard showing upcoming bookings, client information, and session details.

This project gave me a hands-on understanding of how the frontend, server, and database layers depend on each other, and how small decisions in the data model ripple through the whole system.`,
  features: [
    "Online booking flow — package selection, date/time picker, and confirmation in one streamlined process",
    "Service listings with clear presentation of available photography packages and pricing",
    "Client-side form validation in JavaScript for immediate inline feedback",
    "Server-side validation in PHP to ensure data integrity before database writes",
    "MySQL database schema linking users, packages, time slots, and reservations",
    "Admin dashboard for viewing and managing upcoming bookings and client details",
    "Responsive layout built with plain CSS for broad device compatibility",
  ],
  technologies: ["HTML5", "CSS3", "JavaScript", "PHP", "MySQL", "XAMPP"],
  outcome: `Delivered a working end-to-end booking system that handles session scheduling, client data, and reservation management — a practical demonstration of full-stack integration built from the ground up.`,
};

// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function PremiereShotsPage() {
      ...
      <main className="flex-1">
        ...
        <section className="relative min-h-screen flex flex-col justify-center px-8 sm:px-16 lg:px-24 overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-5xl mx-auto">
            ...
            >
              <span className={darkMode ? "text-white" : "text-black"}>Premiere</span>
              <br />
              <span className={darkMode ? "text-white/20" : "text-black/40"}>Shots</span>
            </motion.h1>
        ...
        >
          <div className="max-w-6xl mx-auto">
            ...
            <Link href="/projects/sir-daniels">
              <motion.div
                whileHover={{ x: 4 }}
