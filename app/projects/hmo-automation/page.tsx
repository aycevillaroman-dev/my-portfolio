// app/projects/bmg-hmo-automation/page.tsx
"use client";
...
const project = {
  ...
  subtitle: "Billing File Processor — Internal Tool",
  publishedAt: "2026-04-17",
  summary:
    "A Streamlit-based automation tool that processes monthly billing files, syncs employee data between sheets, and generates structured 7-sheet workbooks for HMO billing operations.",
  images: ["/images/projects/BMG-HMO/bmg1.jpg"],
  team: [
    {
  ...
  overview: `BMG-HMO Billing File Processor is a production-grade Streamlit application designed to automate the complex workflow of processing monthly HMO billing files.

The system handles the complete data pipeline — from validating input files, to syncing employee data between the Attachment and Working File sheets, to generating a structured 7-sheet output workbook with proper formatting, styling, and grand totals.

Built with Python and Streamlit, the application features a memory-efficient architecture with proper workbook cleanup, context managers for safe file handling, and read-only validation mode. It automatically refreshes financial values from the Attachment sheet, appends new employees, and maintains manual entries with visual highlighting.

The output workbook contains Summary, Attachment, Working File, and entity-specific sheets (Sleek, NYFD, BMG Internal, SO) — each with proper formatting, auto-filters, section headers, and grand total rows.`,
  features: [
    "Automated validation of monthly billing files",
    "Intelligent sync between Attachment and Working File sheets",
    "Financial value refresh from Attachment data",
    "Automatic employee appending from Attachment",
    "Entity-based sheet segregation (Sleek, NYFD, BMG Internal, SO)",
    "Manual/WF-only row highlighting for visibility",
    "Master reference database for employee tracking",
    "Interactive Streamlit UI with real-time preview",
    "7-sheet structured output with styling and grand totals",
    "Employee lookup and manual addition capabilities",
  ],
  technologies: [
    "Python",
    "Streamlit",
  ...
  outcome: `Successfully deployed as an internal tool that significantly reduces manual effort in HMO billing processing. Eliminates data entry errors, ensures consistent formatting, and provides real-time visibility into employee data synchronization. The system handles complex workbook operations safely with proper memory management.`,
};
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function BMGHMOPage() {
  ...
  return (
    <div
      className={`font-sans min-h-screen flex flex-col transition-colors duration-500 pb-16 sm:pb-0 ${bg}`}
    >
      {/* ── Floating Dock ─────────────────────────────────────────────────────── */}
      <div className="hidden sm:flex fixed top-4 left-1/2 -translate-x-1/2 z-50">
        <FloatingDock
          ...
          desktopClassName={
            darkMode
              ? "bg-[#111]/90 backdrop-blur-md border border-white/[0.08]"
              : "bg-white/90 backdrop-blur-md border border-black/[0.08]"
          }
          mobileClassName={
            darkMode
              ? "bg-[#111]/90 backdrop-blur-md border border-white/[0.08]"
      ...
      <div
        className={`sm:hidden fixed bottom-0 left-0 right-0 z-50 border-t ${
          darkMode
            ? "bg-[#0a0a0a]/95 backdrop-blur-md border-white/[0.08]"
        ...
        <div className="flex items-center justify-around px-2 py-2">
          {dockItems.map((item, i) => (
            <button
              ...
              title={item.title}
              className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-lg transition-all duration-200 min-w-0 ${
                darkMode
            ...
            >
              ...
              <span
                className={`font-mono text-[8px] uppercase tracking-wider leading-none truncate max-w-[48px] ${
      ...
      <main className="flex-1">
        ...
        <section className="relative min-h-screen flex flex-col justify-center px-8 sm:px-16 lg:px-24 overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-5xl mx-auto">
            ...
            >
              <Link
                href="/"
                className={`inline-flex items-center gap-2 font-mono text-[10px] tracking-[0.2em] uppercase transition-colors ${muted} hover:${
            ...
            <motion.h1
              ...
              className="text-[clamp(3.5rem,12vw,9rem)] font-black tracking-tighter leading-[0.88] mb-6 select-none"
            >
              <span className={darkMode ? "text-white" : "text-black"}>BMG-HMO</span>
              <span className={darkMode ? "text-white/20" : "text-black/40"}> Automation</span>
            </motion.h1>
        ...
        >
          <div className="max-w-6xl mx-auto">
            <div className="mb-20">
              <span
                className={`font-mono text-[10px] tracking-[0.25em] uppercase ${muted}`}
              >
                Project
              </span>
            ...
            <div className="grid md:grid-cols-[1fr_340px] gap-16 items-start">
              ...
              >
                <span
                  className={`font-mono text-[9px] uppercase tracking-[0.25em] ${muted}`}
                >
                  Outcome
                </span>
                ...
                <div
                  className={`flex items-center justify-between pb-5 mb-5 border-b ${borderColor}`}
                >
                  <span
                    className={`font-mono text-[9px] uppercase tracking-widest px-2 py-1 border ${borderColor} ${muted}`}
                  >
                    Python · Streamlit
                  </span>
                  <span className={`font-mono text-xs ${muted}`}>2026</span>
                </div>
        ...
        {/* ── NEXT PROJECT ──────────────────────────────────────────────────────── */}
<section
  className={`border-t ${borderColor} py-28 sm:py-36 px-8 sm:px-16 lg:px-24`}
>
  <div className="max-w-6xl mx-auto">
    <div className="mb-16">
      <span
      ...
      <h2 className={displayHeading}>
        Thanks for
        <br />
        <span className={darkMode ? "text-white/20" : "text-black/15"}>
          Exploring
        </span>
      </h2>
    </div>
    ...
    <Link href="/">
      ...
      >
        <div>
          ...
          <p className={`font-mono text-xs mt-1 ${muted}`}>
            Learn about who I am and what I can do
          </p>
        </div>
        <ArrowRight
          size={20}
