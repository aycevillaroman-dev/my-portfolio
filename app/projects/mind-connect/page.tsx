// app/projects/mind-connect/page.tsx
"use client";
...
const project = {
  title: "MindConnect",
  subtitle: "Mobile Platform for Mental Health Support",
  publishedAt: "2025-09-11",
  summary:
    "A mobile application providing personalized mental health support, advocacy, and guidance with an AI chatbot, self-assessment tools, and community forums.",
  images: ["/mind.png"],
  overview: `MindConnect is a Flutter-based mobile application designed to make mental health support more accessible, personal, and stigma-free. The platform combines AI-driven conversation, structured self-assessment, and peer community into a single cohesive experience — built for users who need guidance but may not have immediate access to professional care.

The backend is powered by Firebase, handling real-time data sync, user authentication, and cloud storage. Firebase Firestore stores user profiles, assessment history, forum posts, and chat logs with offline persistence so the app remains usable without a stable connection. Firebase Authentication manages sign-up and login flows, supporting email/password and anonymous guest sessions to lower the barrier to entry for users who aren't ready to create a full account.

The AI chatbot is built on a Node.js API layer that processes user messages and returns context-aware, empathetic responses tailored to mental health conversations. The chatbot keeps track of session history to maintain conversational continuity, and escalates to resource suggestions or crisis helpline prompts when distress indicators are detected in the conversation.

Self-assessment tools are built around clinically referenced questionnaires — including PHQ-9 for depression screening and GAD-7 for anxiety — presented as interactive Flutter UI flows. Results are stored per-user in Firestore and visualized as progress charts over time, giving users a longitudinal view of their mental health trends. A journaling module lets users log daily moods and reflections, which feed into the same progress dashboard.

The community layer uses Firebase Firestore's real-time listeners to power live group discussions and peer support forums. Posts are organized by topic, moderated through a flagging system, and users can join anonymous or named discussions depending on their comfort level. Direct messaging between users is also supported through the same real-time infrastructure.`,
  features: [
    "AI-powered mental health chatbot with crisis detection",
    "PHQ-9 & GAD-7 self-assessment with progress tracking",
    "Daily mood journaling & longitudinal trend charts",
    "Community forums with anonymous posting support",
    "Real-time peer chat & group discussions",
    "Curated mental health resources & helpline escalation",
    "Offline-capable with Firebase persistence",
    "Guest sessions — no account required to start",
  ],
  technologies: ["Flutter", "Dart", "Firebase", "Firestore", "Node.js", "MongoDB"],
  outcome: `Successfully launched a mobile platform that provides accessible, safe, and personalized mental health support — empowering users to understand and manage their mental health while fostering community and reducing stigma.`,
};
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function MindConnectPage() {
        ...
        <section className="relative min-h-screen flex flex-col justify-center px-8 sm:px-16 lg:px-24 overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-5xl mx-auto">
            ...
            <motion.h1
              ...
              className="text-[clamp(2.8rem,10vw,8rem)] font-black tracking-tighter leading-[0.88] mb-6 select-none"
            >
              <span className={darkMode ? "text-white" : "text-black"}>Mind</span>
              <span className={darkMode ? "text-white/20" : "text-black/40"}>Connect</span>
            </motion.h1>
            ...
            <Link href="/projects/plant-io">
              ...
              >
                <div>
                  ...
                  <h3 className="text-2xl font-black tracking-tight mt-1">Plant.io</h3>
                  <p className={`font-mono text-xs mt-1 ${muted}`}>Web · Community</p>
                </div>
                <ArrowRight
                  size={20}
