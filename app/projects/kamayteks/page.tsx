// app/projects/kamayteks/page.tsx
"use client";
...
const project = {
  title: "KamayTeks",
  subtitle: "Mobile Accessibility App",
  publishedAt: "2025-09-11",
  summary:
    "A mobile application bridging communication between Deaf and hearing individuals through real-time FSL translation, gamified learning, and inclusive accessibility features.",
  overview: `KamayTeks is a mobile accessibility platform built to close the communication divide between Deaf and hearing communities in the Philippines. The core mission is simple but technically demanding: make Filipino Sign Language (FSL) accessible to everyone, in real time, on a device they already carry.

The translation engine runs on a TensorFlow model trained on a curated FSL gesture dataset, processed through a Python backend that handles inference, session state, and result delivery. The React Native frontend captures hand gesture input via the device camera and streams frames to the model, which returns text translations with low enough latency to feel conversational. The reverse flow — text to animated sign — is handled client-side through a library of pre-rendered FSL sequences mapped to vocabulary tokens.

Learning is structured as a progressive module system with gamified progression: users advance through levels, earn points for completed lessons, and unlock new vocabulary sets as they go. Each module combines video demonstrations, interactive practice prompts, and a short assessment. Progress is persisted per user account and visualized on a personal dashboard showing streaks, accuracy trends, and milestone badges.

Accessibility is treated as a first-class concern throughout the interface — not a checkbox. High-contrast modes, adjustable text sizing, haptic feedback, and screen reader support are woven into the component architecture from the start rather than retrofitted. The goal was to build an app that Deaf users could navigate comfortably even before the translation features came into play.

The backend is a lightweight Python service handling model inference, user authentication, progress sync, and push notifications. This keeps the mobile client lean and offloads computation to infrastructure that can scale independently of the app release cycle.`,
  features: [
    "Real-time FSL translation — sign-to-text and text-to-sign via on-device camera",
    "Gamified learning modules with levels, streaks, and milestone badges",
    "Adaptive accessibility: high-contrast mode, haptic feedback, screen reader support",
    "Progress tracking dashboard with accuracy trends and vocabulary stats",
    "Pre-rendered FSL animation sequences for text-to-sign output",
    "Push notifications for daily practice reminders and community updates",
    "Offline-capable core vocabulary module for low-connectivity use",
  ],
  technologies: ["React Native", "Python", "TensorFlow", "Machine Learning", "HTML", "CSS"],
  outcome: `Successfully delivered an accessible communication platform that bridges Deaf and hearing communities through real-time FSL translation and structured learning — demonstrating that inclusive mobile design and on-device AI can coexist without compromise.`,
};

// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function KamayTeksPage() {
        ...
        <section className="relative min-h-screen flex flex-col justify-center px-8 sm:px-16 lg:px-24 overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-5xl mx-auto">
            ...
            <motion.h1
              ...
              className="text-[clamp(3.5rem,13vw,10rem)] font-black tracking-tighter leading-[0.88] mb-6 select-none"
            >
              <span className={darkMode ? "text-white" : "text-black"}>Kamay</span>
              <span className={darkMode ? "text-white/20" : "text-black/40"}>Teks</span>
            </motion.h1>
            ...
            <Link href="/projects/joe-brew">
              ...
              >
                <div>
                  ...
                  <h3 className="text-2xl font-black tracking-tight mt-1">JoeBrew</h3>
                  <p className={`font-mono text-xs mt-1 ${muted}`}>Coffee Shop · Brand Identity</p>
                </div>
